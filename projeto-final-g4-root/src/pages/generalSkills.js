...
function GeneralSkills() {
    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            router.push('/auth'); // Redirect to login if user is not found
        }
    }, [router]);
    ...
    const handleContinue = async () => {
        ...
        const selectedCommunicationSkills = communicationSkill.filter((skill) => skill.checked);
        ...
        const selectedSkills = {
            skills: [...selectedMentalSkills, ...selectedCommunicationSkills].map(skill => skill.name),
        };
    ...
    return (
        <div className="bg-gradient-to-b from-mainBgGradientStart via-mainBgGradientMiddle to-mainBgGradientEnd inline-flex flex-col items-end h-full p-6 text-white">
            {activeView === "mentalSkill" ? (
                <>
                    <div className="text-left mt-20">
                        ...
                        <p className="text-base font-medium mt-6">Identify your specific skills in critical thinking, problem solving and strategic analysis.</p>
                    ...
                    <div className="flex flex-wrap mt-12">
                        {mentalSkill.map((skill, index) => (
                            <div
                                key={index}
                                onClick={() => handleSkillClick(mentalSkill, setMentalSkill, index)}
                                className={`flex flex-wrap p-2 my-2 rounded-full cursor-pointer text-lg font-medium ${skill.checked ? "bg-violet-900" : "bg-slate-800"} text-white`}
                            >
                                {skill.name}
                            </div>
                        ))}
                ...
                <>
                    <div className="text-left mt-20">
                        <h1 className="text-4xl font-semibold">Communication and Interaction Skills</h1>
                        <p className="text-base font-medium mt-6">Assess your communication, collaboration, and empathy skills to build strong relationships.</p>
                        <p className="text-base font-normal mt-2">Select at least 5 skills.</p>
                    </div>
                    <div className="flex flex-wrap mt-12">
                        {communicationSkill.map((skill, index) => (
                            <div
                                key={index}
                                onClick={() => handleSkillClick(communicationSkill, setCommunicationSkill, index)}
                                className={`flex flex-wrap p-2 my-2 rounded-full cursor-pointer text-lg font-medium ${skill.checked ? "bg-violet-900" : "bg-slate-800"} text-white`}
                            >
                                {skill.name}
                            </div>
            ...
            <Button label={activeView === "mentalSkill" ? "Continue" : "Discover my career!"} onClick={handleButtonClick} />
        </div>
    );
}
